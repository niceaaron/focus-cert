...
WITH teacher_certs AS (
	...
	WHERE
		...
		AND CURRENT_DATE::date BETWEEN cflc.log_field4::date
		AND cflc.log_field5::date
),
ESE AS (
	SELECT
...
sched AS (
	SELECT
		cp.school_id,
		cp.course_period_id,
		JSON_ARRAY_ELEMENTS(cp.out_reason::JSON)::TEXT AS reason,
		sc.custom_22 AS tier,
		cp.teacher_id,
		sc.student_id,
	...
	FROM
		...
		AND (
			(
				CAST(CURRENT_DATE AS date) BETWEEN se.start_date
				AND se.end_date
			)
			OR (
		...
		LEFT JOIN (
			SELECT
				...
				ccd.label
			FROM
				PAEC_CCD_CERT_CODES ccd
			UNION ALL
			SELECT
				CONCAT(log.level, log.code) AS code,
				log.description AS label
			FROM
				PAEC_CERT_LOG_OPTIONS log
		) cc ON (
			cc.code = CASE WHEN SUBSTRING(
				(
					STRING_TO_ARRAY(
						TRIM(
							','
							FROM
								mc.certification_requirements
						),
						','
					)
				) [1]
				...
				SUBSTRING(
					(
						STRING_TO_ARRAY(
							mc.certification_requirements, ' and '
						)
					) [1]
					FROM
						1
	...
	WHERE
		...
		AND (
			(
				CAST(CURRENT_DATE AS date) BETWEEN sc.start_date
				AND sc.end_date
			)
			OR (
		...
		AND (cp.custom_28 IN ('O', 'B') or (cp.custom_28 = 'N' and c.short_name !~ '^[A-Za-z]{3}[A-Za-z0-9][0-9]{3}'))
	...
	FROM
		...
		AND (
			EXISTS (
				...
				WHERE
					(
						mp.marking_period_id = sc.marking_period_id
						OR (
							sc.marking_period_id = 0
							AND mp.type = 'year'
						)
					)
					AND mp.syear = {SYEAR}
...
SELECT
	DISTINCT sch.title AS "School Name",
	...
	) THEN 'None' ELSE array_to_string(
		array(
			SELECT
				TRIM(
					LEADING '0123456789 '
	...
	STRING_AGG(
		DISTINCT CASE WHEN sc.reason = '"ELL"'
		AND ell.code = 'LY' THEN 'ESOL' WHEN sc.reason = '"ESE"'
		AND COALESCE(ese.exc, '') NOT IN ('L', '') THEN 'ESE ENDORSEMENT' WHEN sc.reason = '"GIFT"'
		AND COALESCE(ese.exc, '') = 'L' THEN 'GIFTED' WHEN sc.reason = '"REA"'
		AND sc.tier IN ('A', 'B') THEN 'READING ENDORSEMENT' WHEN sc.catalog_cert = 'None' THEN 'None' ELSE TRIM(
			LEADING '0123456789 '
			FROM
				sc.catalog_cert
		) END,
